...
# Natalie Portman
nat.port <- filter(info.merge, actor_1_name == "Natalie Portman" | actor_2_name == "Natalie Portman" | actor_3_name == "Natalie Portman")
nat.port$presence <- "Natalie Portman"
...
actresses.graph5 <- ggplot(data = actresses_data2) + geom_point(mapping = aes(x = reorder(presence, avg_profit_marg), y = avg_profit_marg), color = "orange", size = 2) +
                            geom_segment(mapping = aes(x = presence, xend = presence, y = 0, yend = avg_profit_marg), color = "orange", size = 1, linetype = "dashed") +
                  scale_y_continuous(limits = c(-0.5, 1), labels = percent) +
                  geom_hline(yintercept = 0, color = "black", linetype = "dashed") +
  labs(title = "Acclaimed Actresses' Average Profit Margins Over the Span of Their Careers", 
       subtitle = "We see a similar story with the presence of acclaimed actresses in films \n",
       x = "Actress",  y = "Average Profit Margin", caption = "source: TMBD 5000 & 45,000 Movies (Kaggle.com)") +
...
actresses.graph6 <- ggplot(data = actresses_data2) + geom_point(mapping = aes(x = reorder(presence, avg_profit_marg), y = avg_profit_marg),
                                                                color = ifelse(actresses_data2$avg_profit_marg < 0, "orange", "gray"),
                                                                size = ifelse(actresses_data2$avg_profit_marg < 0, 3, 2)) +
  geom_segment(mapping = aes(x = presence, xend = presence, y = 0, yend = avg_profit_marg),
                color = ifelse(actresses_data2$avg_profit_marg < 0, "orange", "gray"),
                size = ifelse(actresses_data2$avg_profit_marg < 0, 2 , 1)) +
  scale_y_continuous(limits = c(-0.5, 1), labels = percent) +
  geom_hline(yintercept = 0, color = "black", linetype = "dashed") +
  labs(title = "Acclaimed Actresses' Average Profit Margins Over the Span of Their Careers", 
       subtitle = "Even household names like Natalie Portman and Kate Winslet in films did not guarantee a positive profit margin \n",
       x = "Actress",  y = "Average Profit Margin", caption = "source: TMBD 5000 & 45,000 Movies (Kaggle.com)") +
...
actresses.graph7 <- ggplot(data = actresses_data2) + geom_point(mapping = aes(x = reorder(presence, avg_profit_marg), y = avg_profit_marg),
                                                                color = ifelse(actresses_data2$avg_profit_marg > 0.3, "orange", "gray"),
                                                                size = ifelse(actresses_data2$avg_profit_marg > 0.3, 2, 1)) +
  geom_segment(mapping = aes(x = presence, xend = presence, y = 0, yend = avg_profit_marg),
               color = ifelse(actresses_data2$avg_profit_marg > 0.3, "orange", "gray"),
               size = ifelse(actresses_data2$avg_profit_marg > 0.3, 1.5 , 1)) +
  scale_y_continuous(limits = c(-0.5, 1), labels = percent) +
  geom_hline(yintercept = 0, color = "black", linetype = "dashed") +
  geom_hline(yintercept = 0.5, color = "black", linetype = "dashed") +
  labs(title = "Acclaimed Actresses' Average Profit Margins Over the Span of Their Careers", 
       subtitle = "Scarlett Johansson and Meryl Streep are in the minority, but their presence in films have returned profit margins over 50% \n",
       x = "Actress",  y = "Average Profit Margin", caption = "source: TMBD 5000 & 45,000 Movies (Kaggle.com)") +
...
compare_ROI_all <- point_funk(no_big_names, "real_budget", "profit_margin", "marker", "Comparisons Across Films: Impact of Budgets on Profit Margins") +
  labs(subtitle = ("A general theme across the industry suggests: larger budget = larger risk, but also larger returns")) +
  scale_y_continuous(limits = c(-2, 1), labels = percent) +
  scale_x_continuous(trans = "log10", labels = unitLabels, limits = c(10000, 100000000)) +
